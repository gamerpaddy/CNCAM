...
How the thing works inside: what each part does, and why it does it that way
rather than the obvious way. This is the reference for working *on* CNCAM.

For what it is and how to run it, see the [README](../README.md). For what was
originally intended, see [PLAN.md](../PLAN.md).
...
`node src/test/node-run.js` runs everything that does not need a DOM. The rest
(`ui.test.js`, and anything touching `src/app/` or `src/view/`) only runs in the
browser, at `/test.html` on the dev server. Both hosts run the same list from
`src/test/all.js`, so a new test file goes there or it runs nowhere.
